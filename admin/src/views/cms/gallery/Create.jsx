...
function Create() {
  ...
  const fetchData = async () => {
    const response = await basicProvider.getMethod(`cms/gallery/by/${id}`);
    ...
    if (data) {
      if (id) {
        response = await basicProvider.patchMethod(
          `cms/gallery/update/${id}`,
          data
        );
      } else {
        response = await basicProvider.postMethod("cms/gallery/create", data);
        if (response?.data) {
          navigate(`/cms/gallery/${response?.data?._id}/edit`);
        }
      }
    }
